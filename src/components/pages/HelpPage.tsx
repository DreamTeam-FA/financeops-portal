...
const BREAKAGE = [
  ...
  { symptom: "CC Expense Dashboard 'Filter Week:' dropdown shows $0 for all selected weeks", cause: "Column L (Week) in the Raw Data tab is empty — this happens when the sheet was last synced with an older version of the portal that did not include the Week column in its output", fix: "Open the Raw Data tab, type 'Week' in L1, and enter this formula in L2: =ARRAYFORMULA(IFERROR(IF(B2:B=\"\",\"\",TEXT(B2:B-WEEKDAY(B2:B,1)+1,\"mmm d\")&\" \"&CHAR(8211)&\" \"&TEXT(B2:B-WEEKDAY(B2:B,1)+7,\"mmm d, yyyy\")),\"\")). This backfills week labels for all existing rows. Re-syncing from the portal after re-uploading your CSV will permanently fix it with live data." },
  ...
  { symptom: "CC Expenses columns (Account, Description, etc.) show blank after XLSX upload", cause: "QuickBooks exports use short headers ('Date', 'Num', 'Amount') instead of long ones ('Transaction Date'), and the header row may not be in the first 5 rows — both caused column detection to fail", fix: "Fixed in commit 2f339a0 — header search now scans up to 15 rows and recognises both QB-style short headers and standard long headers. Also fixed an empty-string guard bug in the column finder that was mapping all columns to the first empty cell." },
  { symptom: "Portal looks broken on mobile — text appears in vertical orientation, one character per line", cause: "Several badge/chip/pill elements were missing whitespace-nowrap, causing text to wrap character-by-character when the flex container compressed them. Workflows page also had a fixed 220px sidebar on 375px screens leaving ~80px for content. Calendar Week/Month toggle and 4YR Payroll weekLabel were also being squeezed.", fix: "Fixed across commits 6070eb9–710a59a — all chip/badge elements have whitespace-nowrap; Workflows sidebar converts to horizontal nav on mobile; Calendar toggles and Add Event button are nowrap; 4YR Payroll weekLabel and Made-by tag hidden on mobile (they were squeezed to 0px between fixed-width logo and right buttons); Bank Balances chart XAxis rotated to -45° with 60px bottom margin." },
  { symptom: "Paid Bills tab shows bills that are marked 'Paid' in the sheet as 'Unpaid' in the portal (especially TI bills)", cause: "parseAPSheetRows used hardcoded column indices 11 and 12 for status detection fallback regardless of entity. TI's actual status column is col 13 (N), but the code read col 12 (M = Payment Via) instead — so TI paid bills were never detected as paid. The header-scan statusCol is almost always -1 because the data range starts after the header row.", fix: "Fixed in latest commit — status detection now uses getAPColMap(entity).status for the correct per-entity status column. onHold detection also uses the correct per-entity onHold column. All three entities (Ruby's col M=12, MSDx col M=12, TI col N=13) now resolve correctly." },
  { symptom: "Duplicate Bills modal 'Confirm' / 'Cancel' buttons show text vertically (C/o/n/f/i/r/m one letter per line)", cause: "The delete-confirm inline button pair in the Duplicates modal ACTIONS column was missing whitespace-nowrap and shrink-0 — the col-span-2 grid cell compressed them", fix: "Fixed — added whitespace-nowrap to Confirm and Cancel buttons and shrink-0 to their container div in APPage.tsx." },
  { symptom: "Bank Statements 'Mark Downloaded' button shows text vertically on mobile (M/a/r/k one letter per line)", cause: "The toggle button in BankStatementsPage had no whitespace-nowrap and was being compressed by surrounding flex items on 375px viewport", fix: "Fixed — added whitespace-nowrap to the Mark Downloaded / Mark Pending toggle button in BankStatementsPage.tsx." },
                  ...
                  <div className={`rounded-xl border px-4 py-3.5 space-y-2 ${border} ${isLight ? "bg-amber-50" : "bg-amber-900/10"}`}>
                    ...
                    <ul className="space-y-2">
                      {[
                        ...
                        { k: "Mobile calendar: per-day card agenda list", v: 'On mobile (< sm breakpoint), the 7-column month grid is hidden (sm:hidden). Instead, a scrollable per-day card agenda list is shown: only days with at least one event appear; each card shows the day number + weekday, a count badge, and full readable rows for each event (AP bill = vendor + amount, AR = customer + amount, Calendar event = icon + full name + time + assignee + urgency badge). Tapping any event row opens the existing detail modal. Desktop retains the 7-column month grid.' },
                      ].map(({ k, v }) => (
