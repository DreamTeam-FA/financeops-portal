...
const BREAKAGE = [
  ...
  { symptom: "Portal looks broken on mobile — text appears in vertical orientation, one character per line", cause: "Several badge/chip/pill elements were missing whitespace-nowrap, causing text to wrap character-by-character when the flex container compressed them. Workflows page also had a fixed 220px sidebar on 375px screens leaving ~80px for content.", fix: "Fixed across commits 6070eb9, 6c1befd, 4b6dcae — all chip/badge elements now have whitespace-nowrap; the Workflows sidebar converts to a horizontal scrollable nav on mobile (flex-col md:flex-row); the 4YR Payroll filter bar and KPI strip are now compact on mobile; calendar filter buttons and urgency pills use whitespace-nowrap." },
