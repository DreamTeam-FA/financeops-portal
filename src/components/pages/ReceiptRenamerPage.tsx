...
import {
  ...
  ChevronLeft, RotateCcw, Eye, EyeOff, Trash2,
  Download, Sparkles, ScanLine, FileCheck, Loader2,
  ArrowRight, X, Edit3, ChevronDown, Info
} from "lucide-react";
...
export const ReceiptRenamerPage: React.FC<{ onBack?: () => void }> = ({ onBack }) => {
  ...
  const inputCls = `w-full border rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-[#1a73e8] ${isLight ? "bg-slate-50 border-slate-300 text-slate-900" : "bg-[#111] border-[#333] text-white"}`;
  ...
  async function collectHandles(dirH: FileSystemDirectoryHandle, rec: boolean): Promise<FileSystemFileHandle[]> {
    const handles: FileSystemFileHandle[] = [];
    async function walk(dh: FileSystemDirectoryHandle) {
      for await (const [, entry] of (dh as any).entries()) {
        if (entry.kind === "file") {
          const ext = "." + (entry.name as string).split(".").pop()!.toLowerCase();
          if (SUPPORTED_EXTS.has(ext)) handles.push(entry as FileSystemFileHandle);
        } else if (rec && entry.kind === "directory") {
  ...
  const scan = async () => {
    if (!dirHandle) return;
    setStage("scanning");
    setProgress({ current: 0, total: 0, file: "", substage: "Collecting files…" });

    const handles = await collectHandles(dirHandle, recursive);
    if (!handles.length) { setStage("pick"); alert("No PDF or image files found in that folder."); return; }

    setProgress(p => ({ ...p, total: handles.length, substage: "Extracting text…" }));
    ...
    for (let i = 0; i < handles.length; i++) {
      ...
      const newName = buildFilename(vendor, dateObj, total, ext, docType, rawText);
  ...
  const applyRenames = async () => {
    ...
    const updated = [...rows];
    let ok = 0; let skipped = 0; const errors: FileRow[] = [];

    // Learn vendor mappings for rows where user changed vendor field
    ...
    for (const row of toRename) {
      ...
      try {
        if (row.newName === row.original) {
          updated[idx] = { ...updated[idx], status: "done", renamed: false };
          skipped++;
        } else {
        ...
        updated[idx] = { ...updated[idx], status: "error", renameError: e?.message || "Rename failed" };
  ...
  const rebuildName = (row: FileRow, patch: Partial<FileRow>) => {
    ...
    const totalNum = merged.total ? parseFloat(merged.total.replace(/[^0-9.]/g, "")) : null;
    const dt = merged.docType as any;
    const newName = sanitizeFilename(buildFilename(vendor, dateObj, totalNum, merged.ext, dt, merged.rawText));
  ...
  if (!FS_SUPPORTED) return (
    <div className={cl("flex-1 flex flex-col h-full overflow-hidden", bg, text)}>
      <div className="flex items-center gap-3 px-4 py-3 border-b border-orange-500/30 bg-orange-500/10">
        <AlertTriangle className="w-5 h-5 text-orange-400 shrink-0" />
        <div>
          <p className="text-sm font-bold text-orange-300">Browser not supported</p>
          <p className="text-xs text-orange-400/80">Receipt Renamer requires Chrome, Brave, or Edge. Switch browsers to use this tool.</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className={cl("flex-1 flex flex-col h-full overflow-hidden", bg, text)}>
      ...
      <div className={cl("flex items-center gap-3 px-4 sm:px-6 py-3 border-b shrink-0", card, "border")}>
        <button
          onClick={onBack}
          className={cl("p-1.5 rounded-lg transition-colors", isLight ? "hover:bg-slate-100 text-slate-500" : "hover:bg-white/5 text-[#888]")}
        >
        ...
        <div className="ml-auto flex items-center gap-2">
          {stage === "preview" && (
            <>
              <span className={cl("text-[11px]", muted)}>{rows.length} files scanned</span>
              <button
                onClick={openVendorManager}
              ...
              <button
                onClick={() => { setStage("pick"); setRows([]); setDirHandle(null); setDirName(""); }}
                className={cl("p-1.5 rounded-lg transition-colors", isLight ? "hover:bg-slate-100 text-slate-500" : "hover:bg-white/5 text-[#888]")}
                title="Start over"
              >
          ...
          <div className="flex flex-col items-center justify-center min-h-full p-6 gap-6">
            ...
            <div className={cl("w-full max-w-lg rounded-2xl border p-8 text-center", card, "border")}>
              ...
              <p className={cl("text-sm mb-6", muted)}>
                Select a folder of receipts, invoices, or statements. The tool reads each file, extracts vendor, date & amount, then renames everything in place — no uploads, no servers.
              </p>

              {dirName ? (
                <div className={cl("flex items-center gap-2 px-4 py-3 rounded-xl border mb-4 text-left", isLight ? "bg-emerald-50 border-emerald-200" : "bg-emerald-950/30 border-emerald-800/40")}>
                  <FolderOpen className="w-4 h-4 text-emerald-500 shrink-0" />
                  <span className="text-sm font-semibold text-emerald-600 dark:text-emerald-400 truncate">{dirName}</span>
                  <button onClick={() => { setDirHandle(null); setDirName(""); }} className="ml-auto text-emerald-400 hover:text-emerald-600">
                ...
                <button
                  onClick={pickFolder}
                  className="w-full py-4 rounded-xl border-2 border-dashed border-emerald-400/60 hover:border-emerald-400 hover:bg-emerald-500/5 transition-all group mb-4"
                >
                  <FolderOpen className="w-6 h-6 text-emerald-400 mx-auto mb-1.5 group-hover:scale-110 transition-transform" />
                  <p className="text-sm font-semibold text-emerald-500">Click to select folder</p>
                  <p className={cl("text-xs mt-0.5", muted)}>PDF, PNG, JPG, TIFF, HEIC supported</p>
                </button>
              )}

              <label className={cl("flex items-center gap-2 text-xs mb-5 cursor-pointer select-none", muted)}>
                <input
                  type="checkbox"
                  checked={recursive}
              ...
              <button
                onClick={scan}
                disabled={!dirHandle}
                className="w-full py-3.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-600 text-white font-bold text-sm hover:opacity-90 active:scale-[.98] transition-all disabled:opacity-40 disabled:cursor-not-allowed shadow-lg shadow-emerald-500/30 flex items-center justify-center gap-2"
              >
            ...
            <div className="flex flex-wrap justify-center gap-2 max-w-lg">
              {["Renames files in-place", "Learns vendor names", "PDF text + image OCR", "Invoice vs receipt detection", "Editable before applying"].map(f => (
                <span key={f} className={cl("px-3 py-1 rounded-full text-[11px] font-medium border", isLight ? "bg-white border-slate-200 text-slate-500" : "bg-[#0d111a] border-[#1a2235] text-[#888]")}>
                  {f}
                </span>
              ))}
            </div>
            ...
            <div className={cl("flex flex-wrap items-center gap-2 p-3 rounded-xl border", card, "border")}>
              ...
              {(["all", "auto", "review"] as const).map(f => (
                <button
                  key={f}
                  onClick={() => setFilter(f)}
                  className={cl("px-3 py-1.5 rounded-lg text-[11px] font-semibold transition-colors capitalize",
                    filter === f
              ...
              <div className="ml-auto flex items-center gap-1.5">
                ...
                <button
                  onClick={() => setRows(prev => prev.map(r => ({ ...r, selected: r.complete })))}
                  className={cl("px-2.5 py-1.5 text-[11px] font-semibold rounded-lg border",
                    isLight ? "border-slate-200 hover:bg-slate-50 text-slate-600" : "border-[#333] hover:bg-white/5 text-[#aaa]")}
                >
                  Auto only
                ...
                <table className="w-full text-xs min-w-[760px]">
                      ...
                      <th className="w-8 px-3 py-2.5 text-center">
                        <input
                          ...
                          onChange={e => {
                            ...
                            setRows(prev => prev.map(r => ids.has(r.id) ? { ...r, selected: e.target.checked } : r));
                          }}
                  ...
                  <tbody>
                    {filteredRows.map(row => (
                      ...
                      >
                        ...
                        <td className="px-3 py-2">
                          <span className={cl("font-mono text-[11px] truncate block max-w-[160px]", muted)} title={row.original}>
                            {row.original}
                          </span>
                        </td>
                        {/* New name */}
                        <td className="px-3 py-2">
                          <input
                            ...
                            className={cl(
                              "w-full border rounded-md px-2 py-1 text-[11px] font-mono focus:outline-none focus:border-emerald-500 transition-colors",
                        ...
                        <td className="px-2 py-2 text-center">
                          <button
                            onClick={() => setRawPreview({ name: row.original, text: row.rawText || "(no text extracted)" })}
                            className={cl("p-1.5 rounded-md transition-colors", isLight ? "hover:bg-slate-100 text-slate-400" : "hover:bg-white/5 text-[#666]")}
                    ...
                    {filteredRows.length === 0 && (
                      <tr>
                        <td colSpan={9} className={cl("px-4 py-10 text-center text-xs", muted)}>No files match your filter.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
        ...
        {stage === "applying" && (
          <div className="flex flex-col items-center justify-center min-h-full p-8">
            <div className={cl("w-full max-w-md rounded-2xl border p-8 text-center", card, "border")}>
              ...
              <p className={cl("text-xs mt-1", muted)}>Applying changes in-place to your folder</p>
            </div>
          </div>
        )}
        ...
        {stage === "results" && (
          <div className="p-4 sm:p-6 space-y-4 max-w-2xl mx-auto w-full">
            ...
            <div className={cl("rounded-2xl border p-6 text-center", card, "border")}>
              ...
              <p className={cl("text-sm", muted)}>
                {resultSummary.skipped > 0 && `${resultSummary.skipped} skipped (unchanged)`}
                {resultSummary.errors.length > 0 && ` · ${resultSummary.errors.length} errors`}
              </p>
            </div>
              ...
              <div className={cl("rounded-xl border p-4 space-y-2", card, "border border-red-500/20")}>
                ...
                {resultSummary.errors.map(r => (
                  <div key={r.id} className="flex items-start gap-2 text-xs">
                    ...
                    <div>
                      <span className={cl("font-semibold", text)}>{r.original}</span>
                      <span className="text-red-400"> — {r.renameError}</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="flex gap-3">
              <button
                ...
                className={cl("flex-1 py-3 rounded-xl border font-semibold text-sm transition-colors",
                  isLight ? "border-slate-200 hover:bg-slate-50 text-slate-700" : "border-[#333] hover:bg-white/5 text-[#ccc]")}
              >
                ← Back to preview
              </button>
              <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" onClick={() => setRawPreview(null)}>
          <div className={cl("w-full max-w-2xl max-h-[80vh] rounded-2xl border flex flex-col shadow-2xl", card, "border")} onClick={e => e.stopPropagation()}>
            <div className={cl("flex items-center justify-between px-5 py-3 border-b shrink-0", border)}>
              ...
              <button onClick={() => setRawPreview(null)} className={cl("p-1.5 rounded-lg", isLight ? "hover:bg-slate-100 text-slate-500" : "hover:bg-white/5 text-[#888]")}>
                <X className="w-4 h-4" />
              </button>
            </div>
          ...
          <div className={cl("w-full max-w-lg max-h-[80vh] rounded-2xl border flex flex-col shadow-2xl", card, "border")}>
            <div className={cl("flex items-center justify-between px-5 py-3 border-b shrink-0", border)}>
              <div>
                ...
                <p className={cl("text-[11px]", muted)}>Learned mappings — used first when detecting vendors</p>
              </div>
              <button onClick={() => setShowVendorManager(false)} className={cl("p-1.5 rounded-lg", isLight ? "hover:bg-slate-100 text-slate-500" : "hover:bg-white/5 text-[#888]")}>
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              ...
              <div className={cl("rounded-xl border p-3 space-y-2", isLight ? "bg-slate-50 border-slate-200" : "bg-[#111] border-[#222]")}>
                ...
                <input value={learnPattern} onChange={e => setLearnPattern(e.target.value)} placeholder="Pattern (text that appears in the file)…" className={inputCls} />
                <input value={learnName} onChange={e => setLearnName(e.target.value)} placeholder="Canonical name (e.g. McDonalds)…" className={inputCls} />
                <button onClick={addLearnedVendor} className="w-full py-1.5 rounded-lg bg-emerald-500 text-white text-xs font-bold hover:bg-emerald-600 transition-colors">
                  Add to Library
                </button>
              </div>
              ...
              {customVendors.length === 0 ? (
                ...
                customVendors.map(v => (
                  <div key={v.pattern} className={cl("flex items-center gap-3 p-2.5 rounded-lg border", isLight ? "border-slate-100 bg-white" : "border-[#1a2235] bg-[#0d111a]")}>
                    ...
                    <button onClick={() => removeVendor(v.pattern)} className="p-1 rounded hover:bg-red-500/10 text-red-400 hover:text-red-500 shrink-0">
