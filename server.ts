...
const DEFAULT_DATA = {
  ...
  sheetMappings: [
    ...
    { id: "map-statements", module: "statements", name: "Bank Statements Checklist", spreadsheetIdOrUrl: "https://docs.google.com/spreadsheets/d/15uYsYttv4xSYVszpiQh0mtRy7pvoMOxHLMO5KMEmpSs/edit?usp=sharing", tabName: "Bank Statements Data", range: "'Bank Statements Data'!A1:Z200", status: "connected" },
...
app.post("/api/ar/add-item", async (req, res) => {
  ...
  // The largest column index we'll write to (0-based). amtCol is always the rightmost.
  const maxColNeeded = Math.max(amtCol, invCol, appCol, senCol, payCol, remCol, dueCol);

  // Get the sheet's current column count via spreadsheet metadata
  const metaForColUrl = `https://sheets.googleapis.com/v4/spreadsheets/${encodeURIComponent(sid)}`
    + `?fields=sheets.properties(sheetId,title,gridProperties/columnCount)`;
  const metaForColResp = await fetch(metaForColUrl, { headers: { Authorization: `Bearer ${userAccessToken}` } });
  let currentColCount = 70; // fallback if metadata read fails
  let arSheetId: number | null = null;
  if (metaForColResp.ok) {
    const metaForColData: any = await metaForColResp.json();
    const sheetMeta = (metaForColData.sheets || []).find((s: any) =>
      (s.properties?.title || "").toLowerCase() === tabName.toLowerCase()
    );
    if (sheetMeta) {
      currentColCount = sheetMeta.properties?.gridProperties?.columnCount ?? 70;
      arSheetId = sheetMeta.properties?.sheetId ?? null;
    }
  }

  // If the write target exceeds current columns, append enough columns automatically
  const colsNeeded = maxColNeeded + 1; // 0-indexed → 1-indexed count
  if (colsNeeded > currentColCount && arSheetId !== null) {
    const toAdd = colsNeeded - currentColCount + 10; // add a 10-col buffer for future months
    console.log(`[AR/add-item] Auto-expanding "${tabName}" by ${toAdd} columns (current=${currentColCount}, need=${colsNeeded})`);
    const expandUrl = `https://sheets.googleapis.com/v4/spreadsheets/${encodeURIComponent(sid)}:batchUpdate`;
    const expandResp = await fetch(expandUrl, {
      method: "POST",
      headers: { Authorization: `Bearer ${userAccessToken}`, "Content-Type": "application/json" },
      body: JSON.stringify({
        requests: [{ appendDimension: { sheetId: arSheetId, dimension: "COLUMNS", length: toAdd } }],
      }),
    });
    if (!expandResp.ok) {
      const e: any = await expandResp.json().catch(() => ({}));
      return res.status(500).json({ ok: false, error: `Could not expand sheet columns: ${e?.error?.message || expandResp.status}` });
    }
    console.log(`[AR/add-item] Sheet expanded successfully — now has ~${currentColCount + toAdd} columns`);
  }
...
app.post("/api/cc-expense/export-sheet", async (req, res) => {
  ...
  try {
    ...
    if (!spreadsheetId) {
      ...
      const cr: any = await fetch(base, {
        ...
        body: JSON.stringify({
          ...
          sheets: [
            ...
            { properties: { title: "Transaction Details",  sheetId: 204, index: 4, gridProperties: { rowCount: 5000, columnCount: 26 } } },
          ]
        })
      }).then(r => r.json());
      ...
      ytdId    = sid("YTD Summary");
      rawId    = sid("Raw Data");
      detailId = sid("Transaction Details");
    ...
    // ── If syncing: delete and re-add the 4 content tabs ─────────────────────
    if (isSync) {
      const toDelete = [weeklyId, ytdId, rawId, detailId].filter(id => id >= 0);
      const addRequests = [
        ...
        { addSheet: { properties: { title: "Transaction Details", gridProperties: { rowCount: 5000, columnCount: 26 } } } },
      ];
    ...
    //  0        : Title bar
    //  1        : blank
    //  2        : meta (Last Updated | Date Range)
    //  3        : blank
    //  4        : KPI label row (3 boxes)
    //  5        : KPI value row (merged with 6)
    //  6        : KPI bottom padding row
    //  7        : blank spacer
    //  8        : "EXPENSES BY COMPANY" section header
    //  9        : column headers (Company | Amount | % of Total)
    //  10..9+N  : company SUMIF rows (N = COLS.length)
    //  10+N     : TOTAL row
    //  10+N+1   : blank
    //  10+N+2   : note
    const N = COLS.length;
    const coDataStart = 10;          // 0-indexed first company row
    const coDataEnd   = 10 + N;      // 0-indexed TOTAL row
    // 1-indexed sheet rows:
    //   company rows  : 11 .. 10+N
    //   TOTAL row     : 11+N
    const totalSheetRow = 11 + N;    // 1-indexed
    const lastUpdated = new Date().toLocaleString("en-US", { month: "long", day: "numeric", year: "numeric", hour: "numeric", minute: "2-digit" });

    const W = 11; // total dashboard column width (A–K)
    const E = (n: number) => Array(n).fill(""); // empty cells helper
    ...
    const dashValues: any[][] = [
      ...
      // Row 6: KPI bottom padding
      E(W),
      // Row 7: spacer
      E(W),
      // Row 8: section header
      ["EXPENSES BY COMPANY", ...E(W - 1)],
    ...
    // TOTAL row
    dashValues.push(["TOTAL", `=SUM(B11:B${10 + N})`, "100%", ...E(W - 3)]);
    // Spacer + note
    dashValues.push(E(W));
    dashValues.push([...E(4), `Generated by FinanceOps Portal. Use the "Filter Week:" dropdown above to view a specific week. Chart updates automatically.`, ...E(W - 5)]);

    // ── Build Weekly Breakdown values (flat, filterable) ──────────────────────
    // Columns: Week | Week Start | Vendor | [Companies...] | Total | Remarks
    const wkHeaders = ["Week", "Week Start", "Vendor", ...COLS, "Total", "Remarks"];
    const wkValues: any[][] = [wkHeaders];
    for (const row of (weeklyRows || [])) {
      wkValues.push([
        row.weekLabel, row.weekStart, row.vendor ?? "",
    ...
    // ── Build Transaction Details values (individual transactions) ────────────
    // Sort rawData by weekLabel (col 11) then vendor name (col 4)
    const rawDataRows: any[][] = (rawData || []);
    const sortedDetails = [...rawDataRows].sort((a, b) => {
      const wA = String(a[11] ?? ""), wB = String(b[11] ?? "");
      if (wA !== wB) return wA.localeCompare(wB);
      return String(a[4] ?? "").localeCompare(String(b[4] ?? ""));
    });
    const detailHeaders = [...(rawHeaders || [])];
    const detailValues: any[][] = [detailHeaders, ...sortedDetails];
    ...
    await fetch(`${base}/${spreadsheetId}/values:batchUpdate`, {
      ...
      body: JSON.stringify({
        ...
        data: [
          ...
          { range: "Transaction Details!A1", values: detailValues },
        ]
      })
    });
    ...
    const GRAY_TEXT  = { red: 0.25, green: 0.25, blue: 0.25 };
    const AMBER      = { red: 0.949, green: 0.608, blue: 0.071 };
    const PURPLE     = { red: 0.404, green: 0.306, blue: 0.651 };
    ...
    // Box 3: cols 8-10 (Companies) → purple
    const kpiBoxes = [
      { c: 0, bg: BLUE,   valFmt: CURR_FMT },
      { c: 4, bg: GREEN,  valFmt: { type: "NUMBER", pattern: "#,##0" } },
      { c: 8, bg: PURPLE, valFmt: null },
    ];
    for (const { c, bg, valFmt } of kpiBoxes) {
      // Label row (row 4): lighter shade of box color
      const lightBg = { red: Math.min(1, bg.red + 0.35), green: Math.min(1, bg.green + 0.35), blue: Math.min(1, bg.blue + 0.35) };
      requests.push({ mergeCells: { range: R(dashId, 4, 5, c, c + 3), mergeType: "MERGE_ALL" } });
      requests.push({ repeatCell: { range: R(dashId, 4, 5, c, c + 3), cell: { userEnteredFormat: {
        backgroundColor: lightBg, textFormat: { bold: true, fontSize: 9, foregroundColor: WHITE },
        horizontalAlignment: "CENTER", verticalAlignment: "MIDDLE",
      }}, fields: "userEnteredFormat(backgroundColor,textFormat,horizontalAlignment,verticalAlignment)" } });
      // Value row (rows 5-6 merged): main color, big font
      requests.push({ mergeCells: { range: R(dashId, 5, 7, c, c + 3), mergeType: "MERGE_ALL" } });
      const valCell: any = { userEnteredFormat: {
        backgroundColor: bg, textFormat: { bold: true, fontSize: 20, foregroundColor: WHITE },
        horizontalAlignment: "CENTER", verticalAlignment: "MIDDLE",
      }};
      if (valFmt) valCell.userEnteredFormat.numberFormat = valFmt;
      const valFields = valFmt
        ? "userEnteredFormat(backgroundColor,textFormat,horizontalAlignment,verticalAlignment,numberFormat)"
        : "userEnteredFormat(backgroundColor,textFormat,horizontalAlignment,verticalAlignment)";
      requests.push({ repeatCell: { range: R(dashId, 5, 7, c, c + 3), cell: valCell, fields: valFields } });
      // Outer border around the whole box (rows 4-6)
      requests.push({ updateBorders: { range: R(dashId, 4, 7, c, c + 3),
        top: thickBorder(bg), bottom: thickBorder(bg), left: thickBorder(bg), right: thickBorder(bg),
      }});
    }
    // KPI row heights
    ...
    // Row 8: section header "EXPENSES BY COMPANY" — merge left alone (user sets manually); just apply styling to A-C
    ...
    requests.push({ updateDimensionProperties: { range: { sheetId: dashId, dimension: "ROWS", startIndex: 8, endIndex: 9 }, properties: { pixelSize: 28 }, fields: "pixelSize" } });

    // Row 9: column headers — light blue, bold
    requests.push({ repeatCell: { range: R(dashId, 9, 10, 0, 3), cell: { userEnteredFormat: {
      backgroundColor: LIGHT_BLUE, textFormat: { bold: true, fontSize: 10 },
      horizontalAlignment: "LEFT",
    }}, fields: "userEnteredFormat(backgroundColor,textFormat,horizontalAlignment)" } });
    requests.push({ repeatCell: { range: R(dashId, 9, 10, 1, 3), cell: { userEnteredFormat: {
      backgroundColor: LIGHT_BLUE, textFormat: { bold: true, fontSize: 10 },
      horizontalAlignment: "RIGHT",
    }}, fields: "userEnteredFormat(backgroundColor,textFormat,horizontalAlignment)" } });

    // Company data rows (10..9+N): currency on B, % on C, alternating rows
    const coStart = coDataStart, coEnd = coDataEnd;
    for (let i = 0; i < N; i++) {
      const ri = coStart + i;
      const bg = i % 2 === 0 ? { red: 1, green: 1, blue: 1 } : { red: 0.965, green: 0.973, blue: 0.996 };
      requests.push({ repeatCell: { range: R(dashId, ri, ri + 1, 0, 1), cell: { userEnteredFormat: {
        backgroundColor: bg, textFormat: { fontSize: 10 }, horizontalAlignment: "LEFT",
      }}, fields: "userEnteredFormat(backgroundColor,textFormat,horizontalAlignment)" } });
      requests.push({ repeatCell: { range: R(dashId, ri, ri + 1, 1, 2), cell: { userEnteredFormat: {
        backgroundColor: bg, numberFormat: CURR_FMT, textFormat: { fontSize: 10 }, horizontalAlignment: "RIGHT",
      }}, fields: "userEnteredFormat(backgroundColor,numberFormat,textFormat,horizontalAlignment)" } });
      requests.push({ repeatCell: { range: R(dashId, ri, ri + 1, 2, 3), cell: { userEnteredFormat: {
        backgroundColor: bg, numberFormat: { type: "PERCENT", pattern: "0.0%" }, textFormat: { fontSize: 10 }, horizontalAlignment: "RIGHT",
      }}, fields: "userEnteredFormat(backgroundColor,numberFormat,textFormat,horizontalAlignment)" } });
    }
    // Border around entire company table (rows 9..10+N, cols 0-2)
    requests.push({ updateBorders: { range: R(dashId, 9, coEnd + 1, 0, 3),
      top: solidBorder(BLUE), bottom: solidBorder(BLUE), left: solidBorder(BLUE), right: solidBorder(BLUE),
      innerHorizontal: solidBorder({ red: 0.85, green: 0.88, blue: 0.95 }),
    }});

    // TOTAL row (index coEnd): bold gray
    requests.push({ repeatCell: { range: R(dashId, coEnd, coEnd + 1, 0, 1), cell: { userEnteredFormat: {
      backgroundColor: LIGHT_GRAY, textFormat: { bold: true, fontSize: 10 }, horizontalAlignment: "LEFT",
    }}, fields: "userEnteredFormat(backgroundColor,textFormat,horizontalAlignment)" } });
    requests.push({ repeatCell: { range: R(dashId, coEnd, coEnd + 1, 1, 2), cell: { userEnteredFormat: {
      backgroundColor: LIGHT_GRAY, textFormat: { bold: true, fontSize: 10 }, numberFormat: CURR_FMT, horizontalAlignment: "RIGHT",
    }}, fields: "userEnteredFormat(backgroundColor,textFormat,numberFormat,horizontalAlignment)" } });
    requests.push({ repeatCell: { range: R(dashId, coEnd, coEnd + 1, 2, 3), cell: { userEnteredFormat: {
    ...
    if (!isSync) requests.push({ addChart: { chart: {
      spec: {
      ...
      position: { overlayPosition: {
        anchorCell: { sheetId: dashId, rowIndex: 7, columnIndex: 4 },
        widthPixels: 460,
        heightPixels: 320,
      }},
    }}});
    ...
    // Columns: Week | Week Start | Vendor | [Companies...] | Total | Remarks
    // nc_wk = 3 (Week+WeekStart+Vendor) + COLS.length + 1 (Total) + 1 (Remarks)
    const nc_wk = COLS.length + 5; // Week, WeekStart, Vendor, ...companies, Total, Remarks
    const amtColStart = 3;         // first company column (0-indexed)
    const amtColEnd   = 3 + COLS.length + 1; // up to and including Total, exclusive Remarks
    // Header row: blue, bold, white, frozen
    requests.push({ repeatCell: { range: R(weeklyId, 0, 1, 0, nc_wk), cell: cell(BLUE, true, WHITE, "CENTER"), fields: fields(false) } });
    ...
    // Amount columns (companies + total): right-align + currency
    if (amtColEnd > amtColStart) {
      requests.push({ repeatCell: { range: R(weeklyId, 1, wkValues.length, amtColStart, amtColEnd), cell: { userEnteredFormat: { numberFormat: CURR_FMT, horizontalAlignment: "RIGHT" } }, fields: "userEnteredFormat(numberFormat,horizontalAlignment)" } });
    }
    // Remarks column: left-align, wrap text
    requests.push({ repeatCell: { range: R(weeklyId, 1, wkValues.length, nc_wk - 1, nc_wk), cell: { userEnteredFormat: { wrapStrategy: "WRAP", horizontalAlignment: "LEFT" } }, fields: "userEnteredFormat(wrapStrategy,horizontalAlignment)" } });
    ...
    requests.push({ addBanding: { bandedRange: {
      range: R(weeklyId, 1, Math.min(wkValues.length, 4999), 0, nc_wk),
      rowProperties: {
    ...
    requests.push({ setBasicFilter: { filter: { range: R(weeklyId, 0, wkValues.length, 0, nc_wk) } } });
    // Auto-resize Weekly Breakdown
    requests.push({ autoResizeDimensions: { dimensions: { sheetId: weeklyId, dimension: "COLUMNS", startIndex: 0, endIndex: nc_wk } } });
    ...
    // Transaction Details formatting: dark header, freeze, auto-filter, currency on Amount (col 9) and Balance (col 10)
    const detailNc = (rawHeaders || []).length || 12;
    requests.push({ repeatCell: { range: R(detailId, 0, 1, 0, detailNc), cell: cell({ red: 0.133, green: 0.204, blue: 0.369 }, true, WHITE, "CENTER"), fields: fields(false) } });
    requests.push({ updateSheetProperties: { properties: { sheetId: detailId, gridProperties: { frozenRowCount: 1 } }, fields: "gridProperties.frozenRowCount" } });
    for (const ci of [9, 10]) {
      if (ci < detailNc) {
        requests.push({ repeatCell: { range: R(detailId, 1, detailValues.length, ci, ci + 1), cell: { userEnteredFormat: { numberFormat: CURR_FMT, horizontalAlignment: "RIGHT" } }, fields: "userEnteredFormat(numberFormat,horizontalAlignment)" } });
      }
    }
    // Banding for Transaction Details
    requests.push({ addBanding: { bandedRange: {
      range: R(detailId, 1, Math.min(detailValues.length, 4999), 0, detailNc),
      rowProperties: {
        firstBandColor: { red: 1, green: 1, blue: 1 },
        secondBandColor: { red: 0.965, green: 0.972, blue: 0.988 },
      },
    }}});
    requests.push({ setBasicFilter: { filter: { range: R(detailId, 0, detailValues.length, 0, detailNc) } } });
    requests.push({ autoResizeDimensions: { dimensions: { sheetId: detailId, dimension: "COLUMNS", startIndex: 0, endIndex: detailNc } } });
